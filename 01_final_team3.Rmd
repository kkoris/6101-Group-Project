...
```{r,results=TRUE}
library(MASS)
model <- lm(Totcaspopn ~ Population + Blacks +  Menunhlthdays + Smokers, clean)
...
model1 <- lm(Totcaspopn ~ Population + Blacks + Menunhlthdays + Smokers, clean)
summary(model1)
model1$coefficients
...
print('model4')
model4 <- lm(Totcaspopn ~ Population + Blacks, clean)
summary(model4)
model4$coefficients
...
set.seed(1)
mentaltreefit <- rpart(Totcaspopn ~ Menunhlthdays + Blacks + ColEduc + HseCrowded + Smokers, data=clean, control = list(maxdepth = 25) )
# mentaltreefit <- rpart(Kyphosis ~ Age + Number + Start, data=kyphosis, method="class", control = {rpart.control list} )
